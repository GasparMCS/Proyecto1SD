...
# Configuración
BASE_URL = "http://almacenamiento:8000"
ID_URL = f"{BASE_URL}/eventos/getall_ids"
...
# Obtener IDs válidos
ids = requests.get(ID_URL).json()["ids"]
...
while time.time() < fin:
    ...
    # Pausa basada en distribución elegida
        ...
        #pausa = np.random.exponential(1.0 / args.tasa)
    if args.distribucion == "poisson":
...
try:
    stats = requests.get(f"{BASE_URL}/eventos/lru_stats").json()
    print(f"Eventos removidos por LRU (desde backend): {stats['lru_evictions']}")
except:
    print("No se pudo obtener lru_evictions")
